...
public class CollectionManager {
    ...
    public void remove_greater(Vehicle vehicle, Command command){
        ...
        map.entrySet().stream().filter(Vehicle-> Vehicle.getValue().compareTo(vehicle)>0).forEach(Vehicle->map.remove(Vehicle.getKey()));
        if(a>0) {
    ...
    public void remove_key(Integer Key, Command command){
       int a= (int) map.entrySet().stream().filter(Vehicle -> Vehicle.getKey().equals(Key)).count();
        map.entrySet().stream().filter(Vehicle -> Vehicle.getKey().equals(Key)).forEach(Vehicle -> map.remove(Vehicle.getKey()));
if(a>0) {
    ...
    public void remove_any_by_number_of_wheels(Long number, Command command){
       int a= (int) map.entrySet().stream().filter(Vehicle -> Vehicle.getValue().getNumberOfWheels().equals(number)).count();
        map.entrySet().stream().filter(Vehicle -> Vehicle.getValue().getNumberOfWheels().equals(number)).limit(1).forEach(Vehicle -> map.remove(Vehicle.getKey()));
        if(a>0){
